...
export const EditableOrderNotice: React.FC = () => {
  ...
  const move = async (): Promise<void> => {
    ...
    try {
      ...
      notify({
        ...
        description: 'Мы получим одну заявку вместо двух',
      })
      await router.push(`/orders/${order.id}`)
    } catch (cause: unknown) {
      notify({
  ...
  return (
    ...
    >
      Пока сбор открыт, товары можно добавить прямо в неё - тогда мы получим одну заявку вместо
      двух. Эта корзина уйдёт отдельной заявкой, если оформить её.
    </Alert>
  )
}
