 ...
 *
 * Кладёт всегда одну штуку. Количество набирается после добавления — тем
 * степпером, который на странице товара выезжает на освободившееся место
 * (`CartQuantityStepper`): до первого нажатия выбирать число не из чего, а
 * лишний орган управления в строке действий отнимает вес у самого действия.
 *
...
export const AddToCartButton: React.FC<{
  ...
  */
  ...
  if (isInCart) {
    /*
      «Уже в корзине» — круглая белая кнопка в обоих местах, но по разным
      причинам. В карточке каталога состояние читается формой заливки, а не
      длиной подписи: на текст в строке с ценой места нет. На странице товара
      место рядом с кнопкой занимает выехавший степпер, а подпись «В корзине»
      к этому моменту ничего не сообщает — количество на экране и так говорит,
      что позиция там.

      Знак внутри зависит от того, только что товар положили или он лежал там
      и раньше (`CONFIRMATION_MS`): сразу после клика — галочка, дальше —
      корзина.
  ...
  */
  ...
  const button = isCompact ? (
    ...
    >
      {/*
        Без «добавить»: подпись называет, куда товар отправится, и глагол ей
        ничего не добавляет — та же «В корзину» стоит и в шапке. Место для него
        теперь есть (соседнее «в избранное» стало круглым сердцем и ширину
        больше не делит), но разнобой в двух словах об одном и том же — хуже.
      */}
      В корзину
    </Button>
  )
