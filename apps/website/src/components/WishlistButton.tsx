...
export const WishlistButton: React.FC<{
  ...
  if (withLabel) {
    /*
      Уже сохранённое — это ссылка в избранное, а не выключатель. Кнопка с
      подписью стоит на странице товара, куда чаще всего приходят по второму
      разу, и повторное нажатие тихо убирало товар из избранного — ровно
      противоположное тому, чего от неё ждут. Убрать по-прежнему можно, но там,
      где это видно, — на самой странице избранного.
    */
    return (
      <Button
        /*
          ...
          «в избранное»), и сердце рядом с ней ничего не добавляло — в отличие
          от круглой кнопки в карточке, где текста нет вовсе.
        */
        variant="secondary"
        size={size}
        isFullWidth={isFullWidth}
        link={isSaved ? { href: '/wishlist' } : undefined}
        onClick={isSaved ? undefined : onClick}
      >
        {/*
          Без «добавить» — по той же причине, что и у соседней «В корзину»:
          на мобилке кнопки делят строку пополам, и глагол её переполнял.
        */}
