 ...
 * приложение (`_app.tsx`), а на свежей статике выключен и `revalidateOnMount`
 * (`isFreshFallback`), — до возврата во вкладку (`useRefreshOnReturn`) или
 * перезагрузки состояние сбора останется тем, каким приехало. Заявку это не пропустит: `POST /cart/items` отвечает 409
