...
const MESSAGES: Record<string, string> = {
  ...
  */
  ...
  cycle_has_orders: 'В этом сборе уже есть заявки — удалить его нельзя. Закройте сбор вместо этого.',
...
const SCOPED: Partial<Record<ErrorScope, Record<string, string>>> = {
  'cart.add': {
    no_active_cycle:
      'Приём заказов закрыт: сейчас нет открытого сбора. Товар можно будет добавить, когда откроется следующий.',
  ...
  'admin.product.images': {
    unsupported_image_type: 'Подойдут только JPG, PNG или WebP. Пересохраните файл в этих форматах.',
  ...
  'admin.cycles': {
    deadline_must_be_future: 'Дедлайн должен быть в будущем — иначе сбор закроется сразу.',
    cycle_has_orders: 'В сборе уже есть заявки — удалить его нельзя. Закройте сбор вместо удаления.',
  },
  'admin.import': {
