...
const MESSAGES: Record<string, string> = {
  ...
  */
  ...
  admin_only: 'Раздел доступен только админам магазина',

  // --- каталог ---
  product_not_found: 'Товар не найден: возможно, его сняли с продажи. Обновите страницу.',
  /*
    410 от `GET /products/{slug}`: адрес каталогу знаком, но товар снят. Текст —
    страховка на случай, если код всплывёт в интерфейсе: страница товара до него
    не доходит, она отвечает на 410 переадресацией в каталог (`pages/catalog/[slug].tsx`).
  */
  product_gone: 'Этот товар сняли с продажи — посмотрите, что есть в каталоге сейчас.',
  ...
  // Формат заявлен верно, а байты не читаются: файл битый, недокачанный или это
  // вовсе не картинка. Проверяет декодер бэкенда, а не заголовок content-type.
  image_unreadable:
    'Файл не удалось открыть как изображение. Пересохраните его и попробуйте снова.',
  image_too_large: 'Фотография больше 15 МБ. Сожмите её или выберите другую.',
  // Не про размер файла, а про количество пикселей: потолок в 24 Мп бьёт даже по
  // лёгкому файлу, потому что плоский экспорт весит 128 КБ, а разворачивается при
  // декодировании в сотни мегабайт.
  image_too_many_pixels:
    'Слишком большое разрешение — уменьшите фотографию примерно до 4000×4000 и загрузите снова.',
  ...
  // главным, а корзины покупателей оставались бы в первом.
  ...
  // Возвращать нечего или некому: заявку отменил владелец (снимает отмену он сам),
  // последний её товар сняли с продажи (см. `drop_product` на бэке) — или сбор закрылся
  // так давно, что в закупку заявку заведомо не взяли (`PendingStage.UNFULFILLED`).
  // Просто закрытый сбор возврату не мешает: окно у возврата то же, что у отмены.
  order_not_restorable:
    'Вернуть заявку нельзя: её отменил магазин, в ней не осталось товаров или ' +
    'закупка по тому сбору давно прошла',
  // «Отменена покупателем» — утверждение о его действии, и ставить его владелец не может:
  // своя отмена у него называется «Отменена магазином».
  order_status_not_assignable:
    'Этот статус ставит сам покупатель. Чтобы отказать по заявке, выберите «Отменена магазином».',
  ...
  // --- доступ и роли ---
  // Роли раздаёт только super admin. Обычный admin кнопок не видит вовсе, так что
  // сюда приходят разве что открытая с вечера вкладка и запрос руками.
  super_admin_only: 'Выдавать и снимать доступ может только super admin',
  // Роль super admin не меняется ничем — в том числе им самим: иначе магазин
  // остаётся без входа в собственную админку, а обратно пускает только консоль сервера.
  super_admin_immutable:
    'Роль super admin изменить нельзя — это единственный доступ, который магазин не может потерять.',
  super_admin_not_assignable:
    'Super admin в магазине один, и назначается он при настройке — из админки эту роль выдать нельзя.',
  user_not_found: 'Аккаунт не найден — возможно, его удалили. Обновите список.',
  account_not_deletable:
    'Аккаунт с доступом в админку удалить нельзя. Сначала нужно снять роль — после этого аккаунт удаляется как обычный.',
  // Отменить такую заявку покупатель не может — на бэке отмена требует статуса
  // PENDING, — поэтому совет здесь тот же, что на странице профиля и в политике.
  account_has_unfinished_orders:
    'Сначала нужно закрыть заявки, по которым товар уже куплен: подтверждённые и готовые к выдаче. Заберите товары или попросите нас изменить их статус — и аккаунт можно будет удалить.',
...
const SCOPED: Partial<Record<ErrorScope, Record<string, string>>> = {
  'account.delete': {
    // Общий текст говорит «обновите список» — он для админского списка аккаунтов.
    // Здесь удаляют свой собственный, и его уже нет.
    user_not_found: 'Аккаунт уже удалён. Обновите страницу.',
  },
  'cart.add': {
    no_active_cycle:
  ...
  'order.note': {
    order_not_editable: 'Комментарий уже не изменить: сбор закрылся или заявку взяли в работу',
  },
  'order.item.add': {
  ...
  'order.cancel': {
    // Закрытый сбор отмену не закрывает (`isCancellable` против `isEditable`), так что
    // причин отказа две, и обе значат «решение по заявке уже не ваше»: владелец взял её
    // в работу между отрисовкой страницы и нажатием — или закупка прошла мимо неё
    // (`UNFULFILLED`), и отвечать по ней магазину. Текст зовёт написать нам, потому что
    // в обоих случаях это единственное, что осталось сделать.
    order_not_editable:
      'Отменить заявку уже нельзя: решение по ней за магазином. Напишите нам напрямую.',
  },
  'order.restore': {
    // Возврат отдаёт `order_not_restorable`, а не `order_not_editable` (см.
    // `_editing_error` на бэке) — под прежним кодом текст был недостижим. Из трёх
    // причин отказа здесь называем ту, что зависит от времени: остальные две видны
    // на самой странице (кто отменил, что осталось в заявке).
    order_not_restorable:
      'Вернуть эту заявку уже нельзя: закупка по тому сбору давно прошла. ' +
      'Соберите новую в текущем сборе.',
  },
  'admin.orders': {
    // Открытый список — снимок: заявка могла уйти дальше из бота или из соседней
    // вкладки. Тот же код приходит и на «Ожидает» у заявки, опустевшей уже после
    // загрузки страницы (её товары ушли из каталога, возвращать в закупку нечего);
    // в свежем списке такого выбора нет — `StatusSelect` его не покажет.
    order_status_transition_invalid:
      'Так перейти нельзя. Обновите список: статус заявки уже изменился — или в ней не осталось товаров.',
  },
  ...
  'admin.product.images': {
    ...
    image_unreadable: 'Файл повреждён или это не фотография — пересохраните его и попробуйте снова',
    image_too_large: 'Фотография больше 15 МБ — сожмите её и загрузите снова',
    image_too_many_pixels: 'Слишком большое разрешение — уменьшите фотографию до 4000×4000',
...
const SCOPE_FALLBACKS: Record<ErrorScope, string> = {
  ...
  // Удаление либо прошло целиком, либо не начиналось — промежуточного состояния нет
  // (весь разбор аккаунта на бэке в одной транзакции). Поэтому «попробуйте ещё раз»,
  // а не «проверьте, что удалилось».
  'account.delete': 'Не удалось удалить аккаунт. Проверьте связь и попробуйте ещё раз.',
