...
export type ErrorScope =
  ...
  | 'catalog.load'
  | 'catalog.search'
  // пожелание к следующему сбору - форма под пустым поиском
  | 'catalog.wanted'
  // админка
  | 'admin.products.load'
...
const MESSAGES: Record<string, string> = {
  ...
  */
  ...
  /*
    Объёмы товара. Все четыре чинятся в самой форме, поэтому текст называет
    строку таблицы, а не отсылает к каталогу.
  */
  product_variants_empty: 'У товара должен быть хотя бы один объём - с ценой',
  variant_volume_duplicate: 'Один и тот же объём указан дважды - оставьте одну строку',
  too_many_variants: 'Слишком много объёмов у одного товара - не больше 20',
  product_has_variants:
    'У товара несколько объёмов: цену, объём и наличие задаёт таблица объёмов, а не общее поле',
  ...
  // --- пожелания к следующему сбору ---
  // Гость отправил форму, не заполнив имя и номер. Проверка есть и в самой форме,
  // так что сюда приходят разве что запрос руками и вкладка, открытая до выхода
  // из аккаунта: до отправки она считала контакт известным.
  contact_required: 'Оставьте имя и номер телефона - иначе мы не сможем ответить.',
  ...
  // Ссылка на выгрузку (`/export/download/…`) живёт две минуты. Открывает её уже не
  // сайт, а Telegram или браузер, так что текст этот человек видит разве что в JSON.
  export_link_invalid: 'Ссылка на файл устарела. Нажмите «Выгрузить» ещё раз.',
...
const SCOPED: Partial<Record<ErrorScope, Record<string, string>>> = {
  ...
  'catalog.wanted': {
    // Общий текст лимитера говорит «слишком много запросов подряд» - про страницу,
    // которая чем-то занята. Здесь занят человек, и повторять ему нужно то же самое
    // пожелание, а не запрос.
    rate_limited: 'Слишком много пожеланий подряд. Подождите минуту и отправьте ещё раз.',
  },
...
const STATUS_MESSAGES: Record<number, string> = {
  ...
  500: 'Ошибка на сервере. Попробуйте через пару минут.',
...
const SCOPE_FALLBACKS: Record<ErrorScope, string> = {
  ...
  'catalog.wanted': 'Не удалось отправить пожелание',
