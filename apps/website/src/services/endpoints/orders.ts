...
/**
 * Возврат своей отмены. Пока сбор открыт, она обратима: заявка возвращается в
 * «Ожидает подтверждения» тем же составом и с теми же ценами — отмена их не
 * трогала. Отмену владельца эта ручка не снимает: «не смогла достать» — его
 * решение, и отзывает он его сам, из админки.
 *
 * Право на возврат считает бэкенд и отдаёт в `isRestorable`; при отказе
 * приходит 409 `order_not_restorable` — заявку отменил владелец, дедлайн уже
 * прошёл или возвращать нечего.
 */
