...
def stale_orders_for_owner(cycle: OrderCycle, count: int) -> str:
    """Напоминание владельцу: в закрытом сборе висят заявки без ответа.

    Нужно потому, что молчание здесь ничем не отличается от работы: заявка остаётся
    «Ожидает подтверждения», покупатель видит «не вошла в закупку» и ждёт, а в панели
    она выглядит ровно как свежая. Приходит один раз на сбор (`stale_orders_notice_at`):
    это напоминание, а не будильник.

    Сбор назван целиком (`cycle_title`, а не `cycle_mention`): срока в этом сообщении
    нет, и у сбора без подписи от упоминания не оставалось ничего. Просроченных сборов
    к моменту первой рассылки может накопиться несколько, и два «В сборе N заявок»
    подряд — это два напоминания, по которым не понять, какое из них про какой сбор и
    куда в панели идти.

    Второе предложение тоже склоняется: одна заявка — это «подтвердите её», и звать
    отменять «остальные», когда никаких остальных нет, значит заставлять владельца
    перечитывать и искать, что он пропустил.
    """
