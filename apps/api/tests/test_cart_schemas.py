...
def test_add_cart_item_defaults_quantity_to_one() -> None:
    request = AddCartItemRequest(variant_id=uuid.uuid4())
    assert request.quantity == 1


def test_add_cart_item_accepts_camel_case_input() -> None:
    variant_id = uuid.uuid4()
    request = AddCartItemRequest.model_validate({"variantId": str(variant_id), "quantity": 3})
...
def test_add_cart_item_rejects_non_positive_quantity(quantity: int) -> None:
    with pytest.raises(ValidationError):
        AddCartItemRequest(variant_id=uuid.uuid4(), quantity=quantity)
...
def test_cart_quantity_is_capped_like_an_order_line() -> None:
    ...
    with pytest.raises(ValidationError):
        AddCartItemRequest(variant_id=uuid.uuid4(), quantity=MAX_ITEM_QUANTITY + 1)

    with pytest.raises(ValidationError):
        UpdateCartItemRequest(quantity=MAX_ITEM_QUANTITY + 1)

    assert AddCartItemRequest(variant_id=uuid.uuid4(), quantity=MAX_ITEM_QUANTITY).quantity == (
        MAX_ITEM_QUANTITY
    )
