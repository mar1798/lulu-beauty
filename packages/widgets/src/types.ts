...
export type OrderStatus =
  ...
  | 'CANCELLED_BY_OWNER'
/**
 * Где стоит заявка в статусе `PENDING` относительно часов сбора. Не статус и в
 * базе не хранится — бэкенд выводит его из сбора (`PendingStage` в
 * `apps/api/app/orders/models.py`), поэтому он движется сам собой.
 *
 * Нужен потому, что «Ожидает подтверждения» означало две разные вещи: заявку в
 * идущем сборе, которую ещё можно переписать, и заявку из сбора, закрывшегося
 * месяц назад, про которую страница молчала. Разветвляется по нему только текст —
 * что покупателю можно, по-прежнему решают `isEditable`, `isCancellable` и
 * `isRestorable`. У остальных статусов приходит `null`.
 */
export type PendingStage =
  /** Сбор идёт: состав ещё в руках покупателя. */
  | 'COLLECTING'
  /** Сбор закрыт, владелец закупает — обычно несколько дней. */
  | 'PURCHASING'
  /** Окно закупки прошло, ответа нет. Ещё не потеряна, но магазин уже должен слово. */
  | 'DELAYED'
  /** Прошло столько, что заявку в закупку явно не взяли. */
  | 'UNFULFILLED'
...
export interface IOrder {
  ...
  isEditable: boolean
  /**
   * Обратная сторона тех же часов: покупатель отменил заявку сам и вправе свою
   * отмену отозвать. Окно у возврата ровно то же, что у отмены, — иначе промах
   * мышью превращался бы в тупик на шаг дальше; закрывается оно там же, где
   * закрывается отмена: на `UNFULFILLED`. Отмена владельца
   * сюда не попадает: «не смогла достать» — его решение, и снимает его он же, из
   * админки. Вместе с `isEditable` истинным не бывает: заявка либо в работе,
   * либо отменена.
   */
  isRestorable: boolean
  /**
   * Заявку ещё можно отозвать. Это не то же самое, что `isEditable`: правка
   * замирает вместе со списком закупки на дедлайне, а отмена переживает
   * закрытие сбора — против неподтверждённой заявки ничего не куплено. Пока
   * флаг был один, закрытый сбор забирал и то и другое, и у старой заявки не
   * оставалось ни одного действия.
   *
   * На `UNFULFILLED` становится `false`: мимо такой заявки закупка уже прошла,
   * и отмена записала бы молчание магазина как решение покупателя. Дальше
   * отвечает магазин — владельцу об этом напоминает бот.
   */
  isCancellable: boolean
  /** Стадия ожидания — только для `PENDING`, только ради текста. */
...
export interface IHomeHeroProps {
  ...
  description?: string
  /**
   * Метка состояния над заголовком: «сбор открыт», «сбора сейчас нет».
   * Отдельным слотом, потому что сбор — данные сайта, а не шаблона.
   */
  badge?: ReactNode
  /**
   * Таймер до дедлайна — или сообщение, что открытого сбора нет. Стоит под
   * описанием, внутри композиции: до ревизии 3 это был угловой `aside`, и
   * главный признак модели читался как аксессуар.
   */
  status?: ReactNode
  /** Кнопки: «в каталог», «мои заявки». Слот — адреса знает страница. */
  actions?: ReactNode
  /** Мелкая строка под кнопками: счётчики каталога. */
  note?: ReactNode
  /**
   * Правая колонка: витрина товаров текущего сбора. Слот, а не список
   * товаров, — герою про каталог знать не положено, а странице нужна свобода
   * обернуть каждую карточку своим слоем (`Float`, `WishlistButton`).
   *
   * Не задана — герой раскладывается в одну колонку, как до ревизии 3.
   */
  showcase?: ReactNode
  /**
   * Замыкающая плитка витрины (`ShowcaseMore`) — выход в каталог.
   *
   * Отдельным слотом, а не последним элементом `showcase`: она живёт только в
   * ленте на узком экране и прячется, когда витрина становится кластером из
   * трёх карточек. Ширина, на которой это происходит, — дело героя, и знать
...
export interface IFloatProps {
  children: ReactNode
  /**
   * Фаза левитации 0…1. Разводит и стартовую точку, и период (`floatTiming`):
   * соседние элементы не должны качаться в такт. То же число и с тем же
   * смыслом, что `floatPhase` у декоративного пятна.
   */
  phase?: number
  /**
   * Амплитуда, px. Не задана — берётся `SHOWCASE_FLOAT_OFFSET`.
   *
   * Отдельно от фазы: при равной амплитуде соседи ходят по одной дуге и
   * вразнобой, но кластер всё равно читается как один качающийся объект. Ход
   * держите маленьким — под слоем живой текст и кликабельная ссылка.
   */
  distance?: number
}
...
export interface IProductCardProps {
  ...
  /**
   * Грузить фотографию сразу, без ленивой загрузки.
   *
   * Ставится карточкам первого экрана: первой в витрине героя и первому ряду
   * сетки каталога (`IProductGridProps['priorityCount']`) — они оказываются
   * LCP-элементом своей страницы, а о ленивой картинке браузер узнаёт только
   * после раскладки. Всей сетке флаг не ставят: приоритет у всех — это
   * приоритет ни у кого.
   */
  isPriority?: boolean
}

export interface IShowcaseMoreProps {
  /** Надпись: «Смотреть каталог». */
  label: string
  /** Мелкая строка под надписью: например, сколько всего товаров. */
  hint?: string
  /** Куда ведёт плитка. Адрес знает страница. */
  link: ILink
}
...
export interface IProductGridProps {
  ...
  isStaggered?: boolean
  /**
   * Сколько первых карточек грузят фотографию сразу, без ленивой загрузки.
   *
   * Ноль по умолчанию: сетке, которая стоит ниже первого экрана (подборка на
   * главной, избранное), приоритет не нужен и вреден — он отбирает канал у
   * того, что человек видит сейчас.
   *
   * Каталогу нужен: там сетка и есть первый экран, её первая карточка —
   * LCP-элемент страницы, а ленивой браузер узнаёт о ней только после
   * раскладки. Считать надо по самому широкому ряду сетки, а не по всем
   * карточкам: приоритет у всех — это приоритет ни у кого.
   */
...
export interface IProductDetailsProps {
  ...
  /**
   * Количество перед «в корзину». Слот, как и само действие: степпер правит
   * строку корзины, а это состояние `apps/website`. Показывается только у
   * позиции, которая уже в корзине (`isInCart`), — до этого количество
   * выбирать не из чего, штука там ровно одна.
   */
  quantity?: ReactNode
  /**
   * Позиция уже в корзине: количество выезжает слева, а кнопка ужимается в
   * круг со знаком корзины — подпись ей больше не нужна. Обратно строка
   * разворачивается так же, когда позицию убрали.
   */
  isInCart?: boolean
  action?: ReactNode
...
export interface IQuantityStepperProps {
  ...
  label?: string
  /**
   * Подпись кнопки `−`. Задаётся там, где она не уменьшает, а убирает:
   * с `min = 0` шаг вниз с единицы выносит позицию из корзины.
   */
...
export interface ICheckoutPanelProps {
  ...
  buildProductHref: (productSlug: string) => string
  /**
   * Куда вести за тем, чего здесь нет: убрать позицию можно только в корзине.
   * Количество правится на месте (`onQuantityChange`).
   */
  cartHref: string
  ...
  addItem?: ReactNode
  /**
   * Правка количества прямо на оформлении. Без неё состав только читается —
   * так он показан у закрытой заявки и в админке.
   *
   * Удаления здесь нет намеренно: `ItemRow` рисует крестик только вместе с
   * `onRemove`, а промах по нему за секунду до отправки стоил бы дороже
   * перехода в корзину, куда и ведёт `cartHref`.
   */
  onQuantityChange?: (productId: string, quantity: number) => void
  /**
   * Заявка отправляется: степперы замирают, иначе нажатие `+` уходит в
   * корзину, состав которой сервер уже снял в заявку.
   *
   * Занятость отдельной позиции сюда не попадает — количество меняется
   * оптимистично, и гасить его под собственный же запрос значит терять
   * быстрые нажатия (см. `ICartPanelProps.isItemBusy`).
   */
  isBusy?: boolean
