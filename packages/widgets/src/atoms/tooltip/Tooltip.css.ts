...
export const bubble = style({
  ...
  */
  ...
  selectors: {
    /*
      Фокус с клавиатуры — на чём угодно внутри обёртки. Недоступная кнопка
      остаётся в табуляции именно ради этого: на клавиатуре подсказка —
      единственный способ узнать причину.

      `:focus-visible`, а не `:focus-within`: после тапа Android оставляет фокус
      на кнопке, и подсказка у сердечка висела, пока не ткнёшь в другое место.
    */
  ...
  /*
    ...
    Наведение — только там, где оно настоящее: на тач-экране `:hover` после тапа
    залипает до следующего касания.
  */
  ...media({
