 ...
 *
 * Высота колонки ограничена экраном. Прилипший блок не сдвигается вверх при
 * прокрутке, поэтому всё, что не поместилось по высоте, оказывалось ниже края
 * экрана и было недостижимо: на «Товарах» с длинным списком категорий хвост
 * фильтра просто пропадал. Что не влезло — прокручивается внутри `aside`.
 */
export const side = style({
  ...
  ...media({
    lg: {
      ...
      position: 'sticky',
      top: rem(96),
      maxHeight: `calc(100dvh - ${rem(96)} - ${vars.space.lg})`,
    },
  }),
})
 ...
 *
 * Прокручивается здесь, а не на всей колонке: разделы должны оставаться на
 * месте, уезжать под прокрутку имеет смысл только длинному списку фильтров.
 * `minHeight: 0` обязателен — без него флекс-элемент не даёт себя сжать ниже
 * содержимого, и `overflow` не включается.
 */
