...
export const lightTokens = {
  ...
  font: {
    ...
    /**
     * Дисплейная роль — тот же Inter, и это закреплено намеренно.
     *
     * Здесь стоял акцидентный Eloquia Display, но кириллицы в нём нет вовсе
     * (394 глифа, одна латиница). То есть русские заголовки — а на этом сайте
     * других не бывает — набирались не им, а безымянным системным фоллбэком,
     * разным на каждой машине; работал шрифт ровно на названии марки в шапке.
     *
     * Роль при этом осталась отдельным токеном, хоть значение и совпадает с
     * `inter`: появится начертание с кириллицей — правится эта строка, а не
     * два десятка стилей, которые на неё ссылаются.
     */
  ...
  fontSize: {
    ...
    */
    ...
    /*
      Кегль заголовка героя, когда тот раскладывается в две колонки (ревизия 3).
      Отдельная ступень, а не `displaySm`: тот набирает заголовок плашки CTA во
      всю ширину `Container`, и виноват здесь не сам кегль, а множитель —
      `6vw` считается от экрана, тогда как строка стоит в половине его ширины и
      начинает рваться переносами вдвое раньше. `3.2vw` — тот же кегль
      относительно **колонки**; потолок 48px удерживает самую длинную строку
      слогана в одну на 1440.
    */
    displaySplit: 'clamp(1.875rem, 3.2vw, 3rem)',
