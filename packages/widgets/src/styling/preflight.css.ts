...
globalStyle('html', {
  ...
  tabSize: 4,
  /*
    Тема здесь одна — светлая (`themes/light.css.ts`), и сказать об этом браузеру
    надо явно. Иначе при системной тёмной теме UA красит по-тёмному всё, что
    рисует сам: полосы прокрутки, автозаполнение, нативные контролы — и, главное,
    чужие iframe. Кнопка Telegram приезжала с чёрной подложкой в углах скруглений
    ровно поэтому: `color-scheme` наследуется встроенным документом от нас.
  */
  colorScheme: 'light',
})
