...
/**
 * Заголовок — что произошло, коротко («Товар убран из корзины»). Две строки
 * и многоточие — страховка, а не расчёт: название товара в заголовок не
 * кладут, иначе многоточие съедает как раз событие («…убран из…»).
 */
...
/**
 * Предмет уведомления — название товара. Две строки и многоточие: тост висит
 * поверх страницы и уезжает сам, поэтому читают его по диагонали, а длинное
 * название разворачивало карточку на пол-экрана и закрывало то, к чему человек
 * уже вернулся. Обрезанное название узнаётся по началу — ровно за этим оно в
 * тосте и стоит.
 *
 * Пояснение не обрезается: в нём причина ошибки, и второе предложение («обновите
 * страницу») терялось как раз там, где оно нужнее всего.
 */
