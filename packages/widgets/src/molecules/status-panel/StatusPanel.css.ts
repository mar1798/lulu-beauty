...
export const container = style({
  ...
  /*
    Ширина — по содержимому на любом экране.

    До ревизии 3 панель на мобильном растягивалась во всю ширину (`width: 100%`).
    С таймером внутри это читалось не как таймер, а как таблица: три блока цифр,
    разведённые по краям пустой белой полосы во весь экран. Панель — врезка, и
    её край обязан стоять там, где кончается содержимое.

    `fit-content`, а не `max-content`: у варианта с текстом («сбора нет»)
    `max-content` вытянул бы абзац в одну строку и вылез за экран.

    Одного `fit-content` мало: он равен `min(max-content, доступная ширина)`, и
    абзац в две фразы упирается в потолок — то есть снова занимает всю ширину.
    Поэтому потолок задан мерой набора, а не долей экрана: `46ch` — это
    примерно та же длина строки, на которой стоит описание героя, плюс запас на
    горизонтальные поля панели (`box-sizing: border-box`). От `md` потолок
    прежний, 30vw.
  */
  width: 'fit-content',
