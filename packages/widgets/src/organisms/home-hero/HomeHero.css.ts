...
/**
 * Зазор между заголовочным блоком и нижней строкой. Раньше его задавала не
 * величина, а раскладка: `top` висел на `marginBlock: auto`, нижняя строка
 * упиралась в низ, и весь остаток полноэкранной секции доставался этому
 * разрыву — на высоком десктопе между подзаголовком и кнопками зияла
 * половина экрана. Теперь блок центрируется целиком, а разрыв ограничен
 * сверху: на 900px он выходит примерно вдвое меньше прежнего.
 */
const CONTENT_GAP = 'clamp(24px, 10vh, 96px)'
