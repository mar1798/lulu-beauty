...
/**
 * Сетка, которая ждёт новую выдачу: прошлые товары остаются на месте
 * (`keepPreviousData`), но приглушаются, чтобы не читались как ответ на уже
 * набранный запрос. Не скрываются и не блокируются: старая выдача — всё ещё
 * настоящие товары, и открыть карточку из неё никто не мешает.
 *
 * Переход длинный намеренно: на быстром ответе приглушение не успевает
 * набрать силу и мигания не даёт, а на медленном доходит до конца.
 */
