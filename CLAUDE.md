...
- `app/scheduler.py` — the APScheduler wiring (reminder sweep, deadline sweep, OTP cleanup, all on `scheduler_interval_seconds`). The cycle logic itself lives in `app/cycles/scheduler_service.py`. Disable with `SCHEDULER_ENABLED=false`.
...
- **Pages**: public, SSG+ISR — `index.tsx` (home), `catalog/index.tsx` + `catalog/[slug].tsx`; client-rendered, behind login — `cart.tsx`, `checkout.tsx`, `orders/*`, `account.tsx`; auth — `login.tsx`, `register.tsx`, `verify-otp.tsx`, `forgot-password.tsx`, `reset-password.tsx`; owner-only, gated in `getServerSideProps` — `admin/*` (`products/index|add|[id]`, `categories`, `import`, `cycles`, `orders`); `404.tsx`/`500.tsx`.
- **Auth / token handling**: JWTs never reach the browser.
  - `src/server/cookies.ts` — the `lb_at`/`lb_rt` httpOnly cookies (`SameSite=Lax`, `Secure` unless `AUTH_COOKIE_SECURE=false`).
  ...
  - `pages/api/auth/*` — thin proxies to `apps/api`'s `/auth/*` that set/clear those cookies. The catch-all proxy explicitly **404s `/auth/*`**, so token pairs can't leak to the browser through it.
...
- **Data layer** (client): [SWR](https://swr.vercel.app/), configured globally in `_app.tsx` with `revalidateOnFocus: false`.
  ...
  - `src/services/endpoints/*` — one module per domain (`catalog`, `auth`, `cart`, `orders`, `admin`, `cycles`, `export`); all API calls go through these, never raw `fetch` in a page.
