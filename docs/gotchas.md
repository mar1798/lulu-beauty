...
**`Product.variants` is a relationship, and reading it after a plain `select(Product)` is a
`MissingGreenlet`, not a lazy load.** Anything that serializes a product — `product_response`
reads `live_variants` — or walks its volumes has to load them eagerly
(`selectinload(Product.variants)`). The catalogue service, the wishlist, the export and the
import all do; a new query that returns products needs it too.

**`products.price_cents`, `volume_ml` and `in_stock` are derived and will lie if written
directly.** They are recomputed from the live variants by
`ProductService.refresh_display_fields`, which is the only place that should assign them. Set
a price on the product and the next variant edit overwrites it — while the storefront keeps
offering the variant's own price, because that is what the cart charges.

**A product sold in several volumes has `volume_ml IS NULL`**, exactly like a product that has
no volume at all (pads, a sheet mask). The two are told apart by the length of `variants`, not
by the column — a query that reads "no volume" off `volume_ml` alone answers both the same.
