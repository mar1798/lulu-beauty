...
репозитории, сделано; здесь остаётся завести внешний сервис и вписать два
адреса.
...
настоящий `SELECT 1`), `503` — что база недоступна. Именно этот адрес и нужно
дать внешнему монитору (Uptime Kuma на другой машине, healthchecks.io,
UptimeRobot — любому): проверка раз в 1–5 минут, оповещение в Telegram.

Проверять корень домена вместо `/health` бессмысленно: страницы статические и
отдаются даже с мёртвой базой.
